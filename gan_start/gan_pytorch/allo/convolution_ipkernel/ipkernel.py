...
def test_convolution_layer():
    ...
    height_out = (height_after_padding - kernel_height) // stride + 1
    width_out = (width_after_padding - kernel_width) // stride + 1
    ...
    Convolution_layer = nn.Conv2d(channel_in, channel_out, kernel_size=4, stride=stride, padding=padding)
    with torch.no_grad():
        Convolution_layer.weight.copy_(torch.from_numpy(W))
        Convolution_layer.bias.copy_(torch.from_numpy(B))
