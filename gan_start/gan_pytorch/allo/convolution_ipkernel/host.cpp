...
int main(int argc, char** argv) {
    ...
    float in_data_0[49152];
    for (unsigned i = 0; i < 49152; i++) {
      ifile0 >> in_data_0[i];
    }
    size_t size_bytes_in0 = sizeof(float) * 49152;
    std::vector<float, aligned_allocator<float> > source_in0(in_data_0, in_data_0 + 49152);
    ...
    std::vector<float, aligned_allocator<float> > source_in1(in_data_1, in_data_1 + 768);
    ...
    size_t size_bytes_in2 = sizeof(float) * 16;
    std::vector<float, aligned_allocator<float> > source_in2(in_data_2, in_data_2 + 16);
    
    // Directly allocate output buffer with zeros, no need to read from file
    size_t size_bytes_in3 = sizeof(float) * 254016;
    std::vector<float, aligned_allocator<float> > source_in3(254016, 0.0f);
