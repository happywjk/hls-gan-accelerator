...
extern "C" {
  ...
  void top_block1(
    ...
    load_weights_tile(weight_data, shared_weights, gan_block1::IN_CHANNELS, gan_block1::OUT_CHANNELS);
    load_bias_tile(bias_data, shared_bias, gan_block1::OUT_CHANNELS);
    ...
    for (int out_row = 0; out_row < gan_block1::OUTPUT_HEIGHT; out_row++) { 
      for (int out_col = 0; out_col < gan_block1::OUTPUT_WIDTH; out_col++) {
        // 1. Load the input tile for this position
        load_input_tile(input_data, shared_input_tile, out_row, out_col, 
          gan_block1::IN_CHANNELS, gan_block1::INPUT_HEIGHT, gan_block1::INPUT_WIDTH,
          gan_block1::PADDING, gan_block1::STRIDE);
        ...
        kernel_convolution_layer_tile(shared_input_tile, shared_weights, shared_bias, shared_conv_output_tile,
                                     gan_block1::IN_CHANNELS, gan_block1::OUT_CHANNELS);
        ...
        kernel_batchnorm_layer_tile(shared_conv_output_tile, shared_gamma, shared_beta, 
                                   ...
                                   BATCH_SIZE, gan_block1::OUT_CHANNELS, 
                                   MAX_TILE_SIZE, MAX_TILE_SIZE);  // Assuming 1x1 output tile size
        ...
        store_output_tile(output_data, shared_conv_output_tile, out_row, out_col,
                         BATCH_SIZE, gan_block1::OUT_CHANNELS,
                         gan_block1::OUTPUT_HEIGHT, gan_block1::OUTPUT_WIDTH);
      }
    }
  }
}