...
extern "C" {
  ...
  void top_block2(
    ...
    load_weights_tile(weight_data, shared_weights, gan_block2::IN_CHANNELS, gan_block2::OUT_CHANNELS);
    load_bias_tile(bias_data, shared_bias, gan_block2::OUT_CHANNELS);
    load_batchnorm_params(gamma_data, beta_data, running_mean_data, running_var_data,
                          ...
                          gan_block2::OUT_CHANNELS);
    
    // Process each output position
    for (int out_row = 0; out_row < gan_block2::OUTPUT_HEIGHT; out_row++) { 
      for (int out_col = 0; out_col < gan_block2::OUTPUT_WIDTH; out_col++) {
        // 1. Load the input tile for this position
        load_input_tile(input_data, shared_input_tile, out_row, out_col, 
                       gan_block2::IN_CHANNELS, gan_block2::INPUT_HEIGHT, gan_block2::INPUT_WIDTH,
                       gan_block2::PADDING, gan_block2::STRIDE);
        ...
        kernel_convolution_layer_tile(shared_input_tile, shared_weights, shared_bias, shared_conv_output_tile,
                                     gan_block2::IN_CHANNELS, gan_block2::OUT_CHANNELS);
        ...
        kernel_batchnorm_layer_tile(shared_conv_output_tile, shared_gamma, shared_beta, 
                                   ...
                                   BATCH_SIZE, gan_block2::OUT_CHANNELS, 
        ...
        store_output_tile(output_data, shared_conv_output_tile, out_row, out_col,
                         BATCH_SIZE, gan_block2::OUT_CHANNELS,
                         gan_block2::OUTPUT_HEIGHT, gan_block2::OUTPUT_WIDTH);
      }
    }
  }
}