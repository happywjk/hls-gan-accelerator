...
void kernel_tanh_layer_tile(
  ...
  // #pragma HLS FUNCTION_INSTANTIATE variable=reuse_tanh
    ...
    for (int channel = 0; channel < channels; channel++) {
      for (int row = 0; row < height; row++) {
        for (int col = 0; col < width; col++) {
          ...
          float input_value = input_tile[batch][channel][row][col];
          output_tile[batch][channel][row][col] = tanh(input_value);
        }
      }
    }
  }
