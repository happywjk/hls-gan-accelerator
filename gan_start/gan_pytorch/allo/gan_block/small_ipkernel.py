...
def test_genblock():
    ...
    batch_size, channel_in, channel_out = 1, 1, 1
    height, width = 4, 4
    ...
    gamma = np.array([2]).astype(np.float32)
    beta = np.array([2]).astype(np.float32)
    running_mean = np.array([2]).astype(np.float32)
    running_var = np.array([2]).astype(np.float32) + 0.1  # Make sure variance is positive
    ...
    class GenBlock(nn.Module):
        def __init__(self, in_channels, out_channels, padding):
            ...
            self.layers = nn.Sequential(
                nn.ConvTranspose2d(in_channels, out_channels, 2, 2, padding),
    ...
    print(ref[0, 0])  # Print the 4x4 output matrix
